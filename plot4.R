...
#
hpc_feb1_2 <- subset(hpc, Date =="1/2/2007" | Date =="2/2/2007")
...
par(mfcol=c(2,2), xaxp=c(1,2881,2))
...
legend(legend=c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"),
       x="topright", col=c("black", "red", "blue"), lwd=1, bty="n")


with(hpc_feb1_2, plot(Voltage, type="l", lwd=1, 
                xlab="datetime", xaxt="n",col="black"))
axis(side=1, at=tick_indices, labels=tick_labels)

with(hpc_feb1_2, plot(Global_reactive_power, type="l", 
                xlab="datetime", xaxt="n", lwd=1, col="black"))
axis(side=1, at=tick_indices, labels=tick_labels)
